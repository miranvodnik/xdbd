...
inline	static	void	x (XdbdRunningContext *ctx, ctx_appdt_t appdata) \
{ \
	((y*)appdata)->x (ctx); \
} \
void	x (XdbdRunningContext *ctx);
...
inline	static	void	x (XdbdRunningContext *ctx, struct timespec oldTime, struct timespec newTime, long long timeDiff, ctx_appdt_t appdata) \
{ \
	((y*)appdata)->x (ctx, oldTime, newTime, timeDiff); \
} \
void	x (XdbdRunningContext *ctx, struct timespec oldTime, struct timespec newTime, long long timeDiff);
...
inline	static	void	x (XdbdRunningContext *ctx, ctx_timer_t handler, struct timespec t, ctx_appdt_t appdata) \
{ \
	((y*) appdata)->x (ctx, handler, t); \
} \
void	x (XdbdRunningContext *ctx, ctx_timer_t handler, struct timespec t);
...
inline	static	void	x (XdbdRunningContext *ctx, uint flags, ctx_fddes_t handler, int fd, ctx_appdt_t appdata) \
{ \
	((y*)appdata)->x (ctx, flags, handler, fd); \
} \
void	x (XdbdRunningContext *ctx, uint flags, ctx_fddes_t handler, int fd);
...
inline	static	void	x (XdbdRunningContext *ctx, ctx_sig_t handler, siginfo_t* info, ctx_appdt_t appdata) \
{ \
	((y*)appdata)->x (ctx, handler, info); \
} \
void	x (XdbdRunningContext *ctx, ctx_fddes_t handler, siginfo_t* info);
 ...
 *
 */
class XdbdRunningContext
{
public:

	typedef void (*initfunc) (XdbdRunningContext *ctx, ctx_appdt_t appdata);	//!< prototype for starting function of I/O multiplexer
	typedef void (*exitfunc) (XdbdRunningContext *ctx, ctx_appdt_t appdata);	//!< prototype for end function of I/O multiplexer
	typedef void (*timehookfunc) (XdbdRunningContext *ctx, struct timespec oldTime, struct timespec newTime, long long timeDiff,
		ctx_appdt_t appdata);	//!< prototype for time-hook function of I/O multiplexer
	typedef void (*cbfunc) (XdbdRunningContext *ctx, unsigned int flags, ctx_fddes_t handler, int n, ctx_appdt_t appdata);	//!< I/O call-back function prototype
	typedef void (*tmrfunc) (XdbdRunningContext *ctx, ctx_timer_t handler, struct timespec t, ctx_appdt_t appdata);	//!< timer call-back function prototype
	typedef void (*sigfunc) (XdbdRunningContext *ctx, ctx_sig_t handler, siginfo_t *info, ctx_appdt_t appdata);	//!< signal call-back function prototype
...
private:
	...
	public:
		 ...
		 *
		 */
		CSignalContext (XdbdRunningContext* ctx, int signo, sigfunc f, ctx_appdt_t appdata) :
		...
		inline XdbdRunningContext* ctx ()	//!< owner I/O multiplexer instance
		{
			return	m_ctx;
		}
	...
	private:
		...
		XdbdRunningContext*	m_ctx;	//!< reference to owing I/O multiplexer
...
public:
	 ...
	 *
	 */
	inline XdbdRunningContext* sigctx (ctx_sig_t ctx) { return	((CSignalContext*)ctx)->ctx(); }
...
private:
	XdbdRunningContext (bool initialize);
public:
	XdbdRunningContext (initfunc f, exitfunc g, timehookfunc h, ctx_appdt_t appdata, const char* name = 0);
	virtual ~XdbdRunningContext ();
private:
	...
	static void HandleAllSignals (XdbdRunningContext* ctx);
...
private:
	...
	inline void mqLock () { pthread_setcancelstate(PTHREAD_CANCEL_DISABLE, NULL); pthread_mutex_lock(&m_mqLock); }
	inline void mqUnlock () { pthread_mutex_unlock(&m_mqLock); pthread_setcancelstate(PTHREAD_CANCEL_ENABLE, NULL); }

	fd_handler(ReadInternalMessageQueue, XdbdRunningContext)
	fd_handler(WriteInternalMessageQueue, XdbdRunningContext)
...
private:
	...
	static	XdbdRunningContext*	g_initializer;	//!< signal handler initialization object

	int	m_round;	//!< round number of current iteration of I/O multiplexer
	bool	m_quit;	//!< quit indicator set by Quit()

	pthread_mutex_t	m_mqLock;	//!< internal message queue  synchronization lock
 ...
 */
class XdbdRunningContextMsg
{
public:
	XdbdRunningContextMsg () {}	//!< constructor does nothing
	virtual ~XdbdRunningContextMsg () {}	//!< destructor does nothing
	 ...
	 *
	 */
	virtual int Invoke (XdbdRunningContext* ctx) = 0;
 ...
 *
 */
class	SigInfoMsg : public XdbdRunningContextMsg
{
public:
 ...
 *
 */
class	XdbdStopMsg : public XdbdRunningContextMsg
{
public:
	XdbdStopMsg () {}	//!< constructor does nothing
	virtual ~XdbdStopMsg () {}	//!< destructor does nothing
	virtual int Invoke (XdbdRunningContext* ctx);
	virtual const char* Name () { return "XdbdStopMsg"; }	//!< name of this class
};
