...
/*! @brief SQL prepare job construction
 *
 ...
 *
 */
XdbdSqlPrepareJob::XdbdSqlPrepareJob(XdbdWorkingClient* p_workingClient, XdbdShmSegment* p_shmSegment, XdbdRequest* p_request) :
	XdbdSqlJob (p_workingClient, p_shmSegment, p_request, 10 * XdbdBaseJob::NsecPerSec)
{
#if defined(DEBUG)
	if (g_debug) xdbdErrReport (SC_XDBD, SC_ERR, "CREATE XdbdSqlPrepareJob %ld", this);
#endif
	m_sqlParser = 0;
}

/*! brief SQL prepare job destruction
 *
 ...
 *
 */
XdbdSqlPrepareJob::~XdbdSqlPrepareJob()
{
#if defined(DEBUG)
	if (g_debug) xdbdErrReport (SC_XDBD, SC_ERR, "DELETE XdbdSqlPrepareJob %ld", this);
#endif
	delete	m_sqlParser;
	m_sqlParser = 0;
}

/*! brief SQL prepare job execution
 *
 *  function invokes SQL prepare statement. It follows
 *  these steps:
 *
 ...
 *
 */
int XdbdSqlPrepareJob::Execute (XdbdWorkingThread* wt)
{
	...
	do
	{
		...
		sqlStmt = parameters[0].m_stringVal;
		shmSegment->sqlStmt(sqlStmt);
		m_sqlParser = new XdbdSqlParser ();
		if (m_sqlParser->Parse (sqlStmt) != 0)
		{
			m_errorCode = -1;
			m_errorMessage = "syntax errors in SQL statement '";
			m_errorMessage += sqlStmt;
			m_errorMessage += "'";
			break;
		}

#if defined(DEBUG)
		if (g_debug) xdbdErrReport (SC_XDBD, SC_ERR, "Prepare statement: %s", shmSegment->sqlStmt().c_str());
#endif

		if (m_sqlParser->PrepareStatement(shmSegment) != 0)
		{
			m_errorCode = -1;
			m_errorMessage = "cannot prepare SQL statement '";
			m_errorMessage += sqlStmt;
			m_errorMessage += "'";
			break;
		}
 ...
 *
 */
int XdbdSqlPrepareJob::Cleanup ()
{
	workingClient()->first_half_callback(this);
	return	0;
}

/*! @brief report SQL prepare job
 *
 *  another function required by this class's superclass. It reports
 *  kind of job: sql prepare
 *
 */
void XdbdSqlPrepareJob::Report ()
{
	xdbdErrReport (SC_XDBD, SC_ERR, err_info ("job - prepare statement '%s'"), this->request()->XdbdRequest_u.m_sqlRequest.m_parameters.m_parameters_val[0].m_stringVal);
}
